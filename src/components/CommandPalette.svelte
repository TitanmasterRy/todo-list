<script lang="ts">
  ...
  const commands = $derived.by((): Cmd[] => {
    ...
    list.push({ id: 'new-course', label: 'Create course', icon: '➕', run: () => { store.go('courses', { courseId: null }); ui.courseEditor = 'new'; } });
    list.push({ id: 'semester', label: 'Semester setup (add several courses)', icon: '🎓', run: () => { store.go('courses', { courseId: null }); ui.semesterSetup = true; } });
    ...
    list.push({ id: 'motion', label: `Reduced motion: ${store.settings.reducedMotion ? 'on → off' : 'off → on'}`, icon: '🐢', keywords: 'animation toggle', run: () => store.updateSettings({ reducedMotion: !store.settings.reducedMotion }) });
    const nextTheme = store.settings.theme === 'dark' ? 'light' : store.settings.theme === 'light' ? 'system' : 'dark';
    list.push({ id: 'theme', label: `Theme: ${store.settings.theme} → ${nextTheme}`, icon: '🌗', keywords: 'dark light toggle', run: () => store.updateSettings({ theme: nextTheme }) });
    list.push({ id: 'select', label: store.bulkMode ? 'Exit bulk select' : 'Bulk select tasks', icon: '☑️', run: () => (store.bulkMode ? store.clearSelection() : (store.bulkMode = true)) });
