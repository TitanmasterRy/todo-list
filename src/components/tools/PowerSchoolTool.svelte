<script lang="ts">
  ...
  function parse(text: string) {
    ...
    if (!r.courses.length) {
      toasts.push({ message: 'No grades found', detail: 'Save the “Grades and Attendance” page as HTML (Ctrl+S → Webpage, complete) or select the whole table and paste it.', kind: 'warn', timeout: 9000 });
      return;
    }
    parsed = r;
    term = r.terms[r.terms.length - 1] ?? '';
    const m = matchToCourses(r.courses, store.activeCourses.map((c) => ({ id: c.id, name: c.name })));
  ...
  function apply() {
    ...
    toasts.push({ message: `Updated ${updated} course grade${updated === 1 ? '' : 's'}`, detail: created ? `${created} course${created > 1 ? 's' : ''} created. See Tools → Transcript.` : 'See Tools → Transcript.', kind: 'success', emoji: '🏫' });
    parsed = null;
    raw = '';
  }
</script>
...
<section class="card">
  ...
  <p class="help">PowerSchool gives students no API, so this reads the <strong>Grades and Attendance</strong> page you save or copy from the portal and fills course final grades for the transcript and GPA. Nothing is sent anywhere.</p>
  <ol class="steps">
    <li>Log in to PowerSchool and open <strong>Grades and Attendance</strong>.</li>
    <li>Either save the page (<span class="kbd">Ctrl</span>+<span class="kbd">S</span> → “Webpage, complete” or “HTML only”) and upload it, or select the whole grades table, copy, and paste below.</li>
  </ol>
  <div class="btns">
  ...
  {#if parsed}
    <div class="result">
      <div class="row-head">
        ...
        <label class="term">Term <select class="select" bind:value={term}>{#each parsed.terms as t}<option value={t}>{t}</option>{/each}</select></label>
      </div>
      <table>
...
<style>
  ...
  .steps { font-size: 13px; color: var(--text-muted); padding-left: 20px; margin: 0 0 8px; }
  .btns { display: flex; gap: 8px; flex-wrap: wrap; align-items: center; margin: 8px 0; }
  .result { margin-top: 12px; border-top: 1px solid var(--border); padding-top: 10px; }
  .row-head { display: flex; justify-content: space-between; align-items: center; gap: 8px; flex-wrap: wrap; margin-bottom: 8px; }
  .term { display: flex; align-items: center; gap: 6px; font-size: 13px; color: var(--text-muted); }
  .term .select { width: auto; }
